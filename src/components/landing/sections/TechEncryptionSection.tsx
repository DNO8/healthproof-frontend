...
import { Lock, Unlock, KeyRound, ArrowRight, Users } from "lucide-react";
...
export function TechEncryptionSection() {
  ...
  const [stage, setStage] = useState<"idle" | "keygen" | "encrypt" | "transmit" | "decrypt">("idle");
      ...
      <ScrollReveal y={50} duration={0.8}>
        ...
        <div className="mb-6 flex items-center justify-between gap-2">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className={`flex h-14 w-14 items-center justify-center rounded-2xl border-2 transition ${
              currentIdx >= 0 ? "border-sky-300 bg-sky-50" : "border-slate-200 bg-slate-50"
            }`}>
              <Users className="h-6 w-6 text-sky-500" />
            </div>
            <span className="text-xs font-medium text-slate-600">{t("sender")}</span>
          </div>

          {/* Arrow + Key */}
          <div className="flex flex-1 flex-col items-center gap-1">
            <div className="flex w-full items-center gap-1">
              <div className={`h-1 flex-1 rounded-full transition ${currentIdx >= 1 ? "bg-emerald-400" : "bg-slate-200"}`} />
              <div className={`flex h-8 w-8 items-center justify-center rounded-full transition ${
                currentIdx >= 1 ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-400"
              }`}>
                <KeyRound className="h-4 w-4" />
              </div>
              <div className={`h-1 flex-1 rounded-full transition ${currentIdx >= 2 ? "bg-emerald-400" : "bg-slate-200"}`} />
            </div>
            <span className="text-[10px] text-slate-400">
              {currentIdx === 0 ? t("generateKey") : currentIdx >= 1 ? t("sessionKeyActive") : ""}
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            <div className={`flex h-14 w-14 items-center justify-center rounded-2xl border-2 transition ${
              currentIdx >= 3 ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-slate-50"
            }`}>
              <Users className="h-6 w-6 text-emerald-500" />
            </div>
            <span className="text-xs font-medium text-slate-600">{t("receiver")}</span>
          </div>
        </div>
        ...
        <div className="mb-6 min-h-[140px] rounded-2xl border border-(--hp-border) bg-white/60 p-4">
          {stage === "idle" && (
            <div className="flex h-full flex-col items-center justify-center gap-2 py-4 text-center">
              ...
              <p className="text-sm text-slate-500">{t("encryptionIdle")}</p>
            </div>
          )}
          {stage === "keygen" && (
            <div className="space-y-2 py-2">
              <p className="text-sm font-medium text-slate-700">{t("keygenTitle")}</p>
              <div className="flex items-center gap-2 rounded-xl bg-sky-50 px-3 py-2 text-xs font-mono text-sky-700">
                <KeyRound className="h-4 w-4 shrink-0" />
                <span className="break-all">e: 0xA3B5...9F2D · d: [non-extractable]</span>
              </div>
          ...
          {stage === "encrypt" && (
            <div className="space-y-2 py-2">
              <p className="text-sm font-medium text-slate-700">{t("encryptTitle")}</p>
          ...
          {stage === "transmit" && (
            <div className="space-y-2 py-2">
              <p className="text-sm font-medium text-slate-700">{t("transmitTitle")}</p>
          ...
          {stage === "decrypt" && (
            <div className="space-y-2 py-2">
              <p className="text-sm font-medium text-slate-700">{t("decryptTitle")}</p>
