...
export function EncryptionDemo({ onEncrypted }: EncryptionDemoProps) {
  ...
  if (stage === "idle") {
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-(--hp-border) bg-(--hp-layer) p-6 text-center">
        ...
        <p className="text-sm font-medium text-slate-700">{t("encryptFileName")}</p>
        <button
          type="button"
  ...
  if (stage === "encrypting") {
    return (
      <div className="flex flex-col items-center gap-4 rounded-2xl border border-(--hp-border) bg-(--hp-layer) p-6 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-slate-700">{t("encryptingTitle")}</p>
        ...
        <div className="w-full space-y-1.5 rounded-2xl border border-(--hp-border) bg-white/50 p-4 text-left">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t("plaintextLabel")}</p>
          </div>
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/30 p-5 text-center">
      ...
      <p className="text-sm font-semibold text-slate-800">{t("encryptedTitle")}</p>

      <div className="w-full space-y-2 rounded-2xl border border-(--hp-border) bg-white/60 p-4 text-left">
        <div className="flex items-center gap-2">
          <Lock className="h-3.5 w-3.5 text-slate-400" />
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t("ciphertextLabel")}</p>
        </div>
        <p className="break-all font-mono text-[10px] text-slate-600">{ciphertextB64.slice(0, 80)}...</p>

        <div className="flex items-center gap-2 pt-1">
          <Lock className="h-3.5 w-3.5 text-slate-400" />
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t("ivLabel")}</p>
        </div>
        <p className="break-all font-mono text-[10px] text-slate-600">{ivB64}</p>
      </div>

      <div className="w-full space-y-1.5 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 text-left">
        <div className="flex items-center gap-2">
          <Unlock className="h-3.5 w-3.5 text-emerald-500" />
          <p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-600">{t("decryptedLabel")}</p>
        </div>
