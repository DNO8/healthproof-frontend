  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("selectPatient")}</p>
        {DEMO_PATIENTS.map((p, i) => (
          <button
            key={p.id}
            onClick={() => { setSelectedPatient(i); setConfirmed(false); }}
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-slate-400">{t("selectExam")}</p>
        <div className="grid grid-cols-3 gap-2">
          {examTypes.map((exam, i) => (
            <button
              key={exam}
              onClick={() => { setSelectedExam(i); setConfirmed(false); }}
      ...
      <div className="rounded-2xl border border-(--hp-border) bg-(--hp-layer) p-4">
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <p className="text-[10px] font-medium uppercase tracking-wide">{t("orderSummary")}</p>
        </div>
      ...
      {!confirmed ? (
        <button
          type="button"
          onClick={() => { setConfirmed(true); onComplete(); }}
